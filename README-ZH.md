...
如果已有 Skill 可以覆盖大部分流程，不新增。如果只是分支场景，写入已有 Skill。
...
必须检查外部 Skill 是否存在：危险命令、读取敏感信息、上传文件、执行远程脚本、混淆代码、依赖未知 MCP 服务、绕过用户确认等风险。
...
- `install-codex-en.ps1` — 安装 Codex 英文模板
- `install-codex-zh.ps1` — 安装 Codex 中文模板
